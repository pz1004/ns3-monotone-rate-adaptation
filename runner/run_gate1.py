...
# Action set for the learning managers; see run_ablation.py and protocol-v1 A9.1.
# ns3::ThompsonSamplingWifiManager cannot enumerate EHT, so under "Latest" it is run as
# Cqr with propagation disabled -- the path verify_equivalence.py proves byte-identical
# to the shipped sampler. Ideal, MinstrelHt and ConstantRate are already on EHT.
MOD_FAMILY = "Latest"
...
def one_run(cfg):
    ...
    out = tmp / "o.csv"
    eht_ts = (raa == "ThompsonSampling" and MOD_FAMILY == "Latest")
    cmd = [str(BIN), f"--raa={'Cqr' if eht_ts else raa}",
           f"--nSta={nsta}", f"--speed={speed}",
           ...
           f"--modFamily={MOD_FAMILY}", f"--out={out}"]
    if eht_ts:
        cmd += ["--cqrMode=Thompson", "--cqrStructure=Monotone", "--cqrStructWeight=0"]
    if raa == "ConstantRate":
